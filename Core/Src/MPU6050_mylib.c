...
float MPU6050_acc_mes(char axis){
	...
	if(axis == 'x'){
		HAL_I2C_Mem_Read(&hi2c2, MPU6050_RA_ADRR, MPU6050_ACCEL_XOUT_H_16, 1, &Data, 2, 100);
	}
	else if(axis == 'y'){
		HAL_I2C_Mem_Read(&hi2c2, MPU6050_RA_ADRR, MPU6050_ACCEL_YOUT_H_16, 1, &Data, 2, 100);
	}
	else if(axis == 'z'){
		HAL_I2C_Mem_Read(&hi2c2, MPU6050_RA_ADRR, MPU6050_ACCEL_ZOUT_H_16, 1, &Data, 2, 100);
	}
	else{
		return -1;
	}
...
float MPU6050_gyro_mes(char axis){
	...
	if(axis == 'x'){
		HAL_I2C_Mem_Read(&hi2c2, MPU6050_RA_ADRR, MPU6050_GYRO_XOUT_H_16, 1, &Data, 2, 100);
	}
	else if(axis == 'y'){
		HAL_I2C_Mem_Read(&hi2c2, MPU6050_RA_ADRR, MPU6050_GYRO_YOUT_H_16, 1, &Data, 2, 100);
	}
	else if(axis == 'z'){
		HAL_I2C_Mem_Read(&hi2c2, MPU6050_RA_ADRR, MPU6050_GYRO_ZOUT_H_16, 1, &Data, 2, 100);
	}
	else{
		return -1;
	}
...
void MPU6050_print_data(MPU6050_data data){
	printf("ACC_X: %0.3f, ACC_Y: %0.3f, ACC_Z: %0.3f, ", data.acc_x,data.acc_y,data.acc_z);
	printf("GYRO_X: %0.3f, GYRO_Y: %0.3f, GYRO_Z: %0.3f\n\r", data.gyro_x,data.gyro_y,data.gyro_z);
}

void MPU6050_print_raw_data(MPU6050_data data){
